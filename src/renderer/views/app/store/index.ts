...
export class Store {
    ...
    public isIncognito = window.ipcRenderer.sendSync(
        `is-incognito-${this.windowId}`
    )
    ...
    public constructor() {
        ...
        window.ipcRenderer.on("update-navigation-state", (e, data) => {
            this.navigationState = data
        })

        window.ipcRenderer.on("is-ui-page", (e, data) => {
            this.isUIpage = data
        })

        window.ipcRenderer.on("update-navigation-state-ui", (e, url) => {
            var url = url.url
            this.isUIpage =
                url.startsWith(WEBUI_BASE_URL) ||
                url.startsWith(NETWORK_ERROR_HOST)
        })

        window.ipcRenderer.on("fullscreen", (e, fullscreen: boolean) => {
            this.isFullscreen = fullscreen
        })

        window.ipcRenderer.on("html-fullscreen", (e, fullscreen: boolean) => {
        ...
        window.ipcRenderer.on("dialog-visibility-change", (e, name, state) => {
