...
export class TabsStore {
    ...
    public constructor() {
        ...
        window.ipcRenderer.on(
            "create-tab",
            (e, options: TabCreateProperties, isNext: boolean, id: number) => {
                if (isNext) {
    ...
    @action public createTabs(options: TabCreateProperties[], ids: number[]) {
        this.removedTabs = 0
    ...
    public calculateTabMargins() {
        ...
        for (const tab of tabs) {
            ...
            if (tab.tabGroupId !== currentGroup) {
                if (tab.tabGroup) {
                    tab.marginLeft =
                        tab.tabGroup.placeholderRef.current.offsetWidth + 16
                } else {
                    tab.marginLeft = 6
                }
    ...
    public onMouseMove = (e: any) => {
        ...
        if (this.isDragging) {
            ...
            const { tabStartX, mouseStartX, lastMouseX, lastScrollLeft } =
                store.tabs
