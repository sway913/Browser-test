...
export class DialogStore {
    ...
    public constructor(
        ...
        if (visibilityWrapper && persistent) {
            window.ipcRenderer.on("visible", async (e, flag, ...args) => {
        ...
        window.ipcRenderer.on("update-tab-info", (e, tabId, data) =>
    ...
    public async invoke(channel: string, ...args: any[]) {
        return await window.ipcRenderer.invoke(`${channel}-${this.id}`, ...args)
    }

    public send(channel: string, ...args: any[]) {
        window.ipcRenderer.send(`${channel}-${this.id}`, ...args)
    }
